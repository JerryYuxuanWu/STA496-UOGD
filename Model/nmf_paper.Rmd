...
#hourly_data<- readRDS("/Users/meredith/Library/CloudStorage/GoogleDrive-mereditf@usc.edu/Shared drives/HEI Energy/flaring/new-git/STA496-UOGD/data/processed_data/TrailerProcessed20251018.rds")

wd<-"/Users/meredith/Library/CloudStorage/GoogleDrive-mereditf@usc.edu/Shared drives/HEI Energy/data/final data/LNM_Finalized_Data 2/VOC sampling window data"
wd <- '../data/VOC sampling window data 20251018/'
# File names
filenames <- c(
...
hourly_data <- hourly_data %>% rename('co2' = 'co2_ppm')  %>%
  ...
  mutate(day = as.Date(datetime_mountain),
         daytime = ifelse(hour(datetime_mountain) >= 11 & hour(datetime_mountain) <= 17, 1, 0))


hourly_data$total_radioactivity<-hourly_data$radon_B+hourly_data$rd_particle_B

#write.csv(hourly_data,"/Users/meredith/Library/CloudStorage/GoogleDrive-mereditf@usc.edu/Shared drives/HEI Energy/data/final data/hourly_merged_9-8-2025.csv", row.names = F)
...
hourly_nona <- hourly_data %>% 
  dplyr::select(any_of(c('day', 'time_utc', 'daytime', vocs, non_vocs, 'wdr_deg', 'wsp_ms'))) %>% 
  na.omit()
...
#  pull(yearmonth)
#write.csv(hourly_full_nona, "/Users/meredith/Library/CloudStorage/GoogleDrive-mereditf@usc.edu/Shared drives/HEI Energy/data/final data/hourly_nona_merged_10-18-2025.csv", row.names = FALSE)
``` 
...
replace_zero_with_random <- function(column, name, LOD_df){
  ...
  column <- if_else(column == 0, round(runif(length(column), 0, 0.5 * LOD), 3), column)
  column <- if_else(column == 0, min(column[column!=0]), column)
  return (column)
}
...
```{r}
# Save for my comp paper
# saveRDS(normalized_matrix_less_o3, 'normalized_matrix_less_o3.rds')
# saveRDS(uncertainty_matrix, 'uncertainty_matrix.rds')
```
...
basis_matrix_5c_less_o3 <- as.matrix(basis_matrix_5c_less_o3)

normalized_hourly_data_5c_less_o3 <- hourly_nona[,c('day', 'time_utc')] %>%
  cbind(normalized_matrix_less_o3) %>%
  cbind(basis_matrix_5c_less_o3) %>%
  right_join(hourly_data %>% dplyr::select(-'day'), join_by(time_utc), suffix = c('_norm', ''))

#saveRDS(normalized_hourly_data_5c_less_o3, '/Users/meredith/Library/CloudStorage/GoogleDrive-mereditf@usc.edu/Shared drives/HEI Energy/papers/nmf/normalized_hourly_data_5c_less_o3.rds')
#saveRDS(normalized_hourly_data_5c_less_o3, '/Users/meredith/Library/CloudStorage/GoogleDrive-mereditf@usc.edu/Shared drives/HEI Energy/flaring/new-git/STA496-UOGD/Model/result_rfiles/normalized_hourly_data_5c_less_o3.rds')
...
nmfplt_2_svd_5c_less_o3 <- get_component_plot(H_long_5c_less_o3, 
                                           '2', '5) Internal Combustion and Soil Respiration')
...
```{r}
get_component_plot(H_long_5c_less_o3, '1', '1')
get_component_plot(H_long_5c_less_o3, '2', '2')
get_component_plot(H_long_5c_less_o3, '3', '3')
get_component_plot(H_long_5c_less_o3, '4', '4')
get_component_plot(H_long_5c_less_o3, '5', '5')
```
...
final_plot
# ggsave("/Users/meredith/Library/CloudStorage/GoogleDrive-mereditf@usc.edu/Shared drives/HEI Energy/papers/nmf/factors_patchwork_5factor_10-18-2025.png", final_plot, width = 16, height = 20)
...
factor_labels <- c(
  ...
  "Factor 2" = "Combustion/Respiration",
  "Factor 3" = "Flaring",
  "Factor 4" = "Traffic",
...
#df_wide <- contrib_prop %>%
#  pivot_wider(
#    id_cols = Chemical,
#    names_from = Component,
#    values_from = Contribution_prop
#  )

# Find dominant factor and proportion
#df_wide <- df_wide %>%
 # rowwise() %>%
#  dplyr::mutate(
#    DominantFactor = names(.)[which.max(c_across(starts_with("Factor")))],
#    DominantProportion = max(c_across(starts_with("Factor")), na.rm = TRUE)
#  ) %>%
 # ungroup()

# Rename columns for factors
#colnames(df_wide) <- ifelse(colnames(df_wide) %in% names(factor_labels),
#                            factor_labels[colnames(df_wide)],
#                            colnames(df_wide))

# Replace dominant factor values with descriptive labels
#df_wide <- df_wide %>%
#  dplyr::mutate(DominantFactor = factor_labels[DominantFactor])


#write.csv(contrib_prop,"/Users/meredith/Library/CloudStorage/GoogleDrive-mereditf@usc.edu/Shared drives/HEI Energy/papers/nmf/contribution_proportions.csv",row.names = FALSE)

#write.csv(df_wide,"/Users/meredith/Library/CloudStorage/GoogleDrive-mereditf@usc.edu/Shared drives/HEI Energy/papers/nmf/dominant_proportions.csv",row.names = FALSE)
...
ggsave("/Users/meredith/Library/CloudStorage/GoogleDrive-mereditf@usc.edu/Shared drives/HEI Energy/papers/nmf/fingerprint_5factor_10-18-2025.png", fingerprint)
```

```{r}
hourly_wind_nona <- hourly_nona %>%
  dplyr::select(wdr_deg, wsp_ms)
compass_labels<- tibble(
 wd = c(0, 45, 90, 135, 180, 225, 270, 315, 360),
 label = c("N", "NE", "E", "SE", "S", "SW", "W", "NW", "N")
)
get_wind_plot_data <- function(W,
                    factor_names = c('Fugitive/Venting',
                    'Combustion/Respiration',
                    'Flaring',
                    'Traffic',
                    'Produced Water'
...
get_wind_plots <- function(W,
  ...
  factor_labels <- setNames(paste(paste0(1:5, ')'), factor_names[factor_order]),
...
wind_plot <- get_wind_plots(
  ...
  y_axis_upper = c(0.12, 0.12, 0.12, 0.12, 0.12),
  factor_order = c(1, 5, 4, 3, 2))
...
wind_plot[[5]] <- wind_plot[[5]] + labs(x = "Wind Direction (°)") +
...
ggsave("/Users/meredith/Library/CloudStorage/GoogleDrive-mereditf@usc.edu/Shared drives/HEI Energy/papers/nmf/wind_factor_plots_patchwork_5factor_10-18-2025.png", final_plot, width = 8, height = 10)
```

- Table of contribution proportions

```{r}

factor_labels <- c("Fugitive/Venting",
                   "Combustion/Respiration",
                   "Flaring",
                   "Traffic",
                   "Produced Water")
...
chemical_labels <- c(
  ...
  "o-xylene" = "o-Xylene", "m&p-xylene" = "m&p-Xylene",
  "co" = "CO", "co2" = "CO2", "nox" = "NOx",
  "h2s" = "H2S", "so2" = "SO2", "o3" = "O3", "ch4" = "CH4",
  "total_radioactivity" = "Radioactivity"
)

# ----- 1) Long format (Factor row label = H_df_5c_less_o3$Component) -----
df_long <- H_df_5c_less_o3 %>%
  pivot_longer(
    cols = -Component,
    names_to = "Species",
...
# ----- 2) Map the factor row labels to your 1–5 labels -----
# This handles Components like "1", "Factor1", etc. If already named ("Traffic"), it leaves as-is.
df_long <- df_long %>%
  mutate(
    .idx = suppressWarnings(as.integer(gsub("\\D+", "", Component))),
    Factor = ifelse(!is.na(.idx) & .idx >= 1 & .idx <= length(factor_labels),
                    factor_labels[.idx],
                    as.character(Component))
  ) %>%
  select(-.idx)

# ----- 3) Wide to species × factors -----
df_wide <- df_long %>%
  select(-Component) %>%
  pivot_wider(names_from = Factor, values_from = Contribution)

# Ensure factor columns are numeric (in case anything came in as character)
factor_cols <- setdiff(names(df_wide), "Species")
df_wide <- df_wide %>%
  mutate(across(all_of(factor_cols), ~ suppressWarnings(as.numeric(.))))

# ----- 4) Compute dominant factor + proportion (robust, fast) -----
mat <- as.matrix(df_wide[factor_cols])
# Handle rows that are all NA safely
all_na <- apply(mat, 1, function(x) all(is.na(x)))
# Replace all-NA rows with very small numbers so max.col won't break (we'll set NAs back)
mat_safe <- mat
mat_safe[is.na(mat_safe)] <- -Inf
idx <- max.col(mat_safe, ties.method = "first")
idx[all_na] <- NA_integer_

final_table <- df_wide %>%
  mutate(
    DominantFactor = ifelse(is.na(idx), NA_character_, factor_cols[idx]),
    DominantProportion = ifelse(is.na(idx), NA_real_, mat[cbind(seq_len(nrow(mat)), idx)])
  )

# ----- 5) Optional: pretty species names + rename to "Component" and reorder columns -----
final_table <- final_table %>%
  mutate(Component = dplyr::recode(Species, !!!chemical_labels, .default = Species)) %>%
  select(Component, all_of(factor_cols), DominantFactor, DominantProportion)
