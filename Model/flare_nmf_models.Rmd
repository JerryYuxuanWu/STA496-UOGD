...
### Try to merge V3.5
```{r}
vnf_v35 <- readRDS('../DataProcessing/vnf_cleaned_20230501-20240601.rds')
flaring_loving_50 <- vnf_v35 %>%
  select(all_of(c('date', 'temp_bb', 'area_bb', 'v35_methane_eq' = 'methane_eq', 'lon', 'lat'))) %>%
  right_join(flaring_loving_50, join_by(date, temp_bb, area_bb, lon, lat)) %>%
  arrange(date)
```

### Get daily flare data
```{r}
...
for (r in radius) {
  ...
  summarise(flare_wd_day = sum(abs(angle - wdr_deg.day) <= 30, na.rm=T),
            ...
            methane_eq = mean(-7.15+1.08*10^-2*temp_bb-5.31*10^-6*temp_bb^2+8.66*10^-10*temp_bb^3+1.2^10^-2*area_bb)) %>%
