...
## NMF data
```{r}
nmf_data <- readRDS('result_rfiles/normalized_daily_data_5c_less_o3.rds')
names(nmf_data)
```
```{r}
nmf_factor <- nmf_data %>%
  select(day, Factor1, Factor2, Factor3, Factor4, Factor5) %>%
  mutate(datetime_mountain = with_tz(as.POSIXct(day, tz = 'UTC', format = "%Y-%m-%d"), 
                                     tzone = "America/Denver")) %>%
  mutate(day_mountain = as.Date(format(datetime_mountain, '%Y-%m-%d'))) %>%
  select(-day, -datetime_mountain)
```
...
full_merged <- trailer_daily %>%
  ...
  left_join(nmf_factor, join_by(day_mountain))
