...
- Normalize then replace zero
- STEP 3: Normalize the non-vocs
...
hourly_nona_bgrm_norm <- as_tibble(sapply(as.list(hourly_nona_bgrm), normalize_column)) 

summary(hourly_nona_bgrm_norm)
```

- STEP 4: replace zero values with random values between 0 and 0.5xLOD

```{r}
set.seed(123)
replace_zero_with_random <- function(column, name, LOD_df){
  LOD <- LOD_df$LOD[LOD_df$chemical == name]
  column <- if_else(column == 0, round(runif(length(column), 0.001, 0.5 * LOD), 3), column)
  return (column)
}

hourly_nona_bgrm_norm_zerorepl <- hourly_nona_bgrm_norm %>%
  mutate(across(adjusted_background_non_voc$chemical, 
                ~ replace_zero_with_random(.x, cur_column(), adjusted_background_non_voc)))

hourly_nona_bgrm_norm_zerorepl <- hourly_nona_bgrm_norm_zerorepl %>%
  mutate(across(adjusted_background_voc$chemical, 
                ~ replace_zero_with_random(.x, cur_column(), adjusted_background_voc)))

summary(hourly_nona_bgrm_norm_zerorepl)
```

- FINAL step: create matrix of processed and normalized concentrations for NMF

```{r}
normalized_matrix <- as.matrix(hourly_nona_bgrm_norm_zerorepl) 
...
LOD_merged <- tibble(chemical = names(hourly_nona_bgrm_norm_zerorepl)) %>%
...
# # # 19.18 minutes to run the above
# 
# saveRDS(lsnmf_random_less_o3,
#         'lsnmf_random_less_o3_rl0last.rds')

lsnmf_random_less_o3 <- readRDS('lsnmf_random_less_o3_rl0last.rds')
```
...
# # 1.42 minutes to run the above
# 
# saveRDS(lsnmf_nndsvd_less_o3,
#         'lsnmf_nndsvd_less_o3_rl0last.rds')

lsnmf_nndsvd_less_o3 <- readRDS('lsnmf_nndsvd_less_o3_rl0last.rds')
```
...
# # 14.7 minutes to run the above
# 
# saveRDS(kl_random_less_o3, 'kl_random_less_o3_rl0last.rds')

kl_random_less_o3 <- readRDS('kl_random_less_o3_rl0last.rds')
```
```{r}
# plots the NMF rank survey
...
# saveRDS(kl_nndsvd_less_o3, 'kl_nndsvd_less_o3_rl0last.rds')

kl_nndsvd_less_o3 <- readRDS('kl_nndsvd_less_o3_rl0last.rds')
```
```{r}
# plots the NMF rank survey
