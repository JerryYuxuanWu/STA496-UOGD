...
print(paste0("Variance explained by each factor : ", round(variance_explained_trad_factors[1], 4), ', ', 
            paste(round(diff(variance_explained_trad_factors), 4), collapse=', ')))
factor_var_expl <- c(variance_explained_trad_factors[1], diff(variance_explained_trad_factors))
# Old data order 5, 3, 2, 4, 1
variance_explained_pkg
variance_explained_pkg_factors
print(paste0("Variance explained by each factor (pkg): ", round(variance_explained_pkg_factors[1], 4), ', ', 
            paste(round(diff(variance_explained_pkg_factors), 4), collapse=', ')))
```

#### Alternative way of variance explained
Consider the variance lost from removing each factor. 

Suppose we only have the other 4 factors, then this variance is computed as:
- Reconstruct the matrix with these 4 factors
- Compute WRSS and Variance Explained of this lower rank reconstruction matrix
- Find difference in WRSS and Variance Explained between this lower rank reconstruction matrix and original reconstruction matrix.

This difference would be the "Variance Explained" gained from having this factor.
If we think of NMF as part decompositions, then we can think of this as how much variance is explained with this additional part (factor). 
```{r}
...
paste('Variance explained from each factor (LOO + traditional WRSS): ', 
      paste0(order, ' (',round(variance_explained_trad_factors_loo[order], 3),')', collapse = ', '))
...
paste('Variance explained from each factor (LOO + package WRSS): ', 
      paste0(order, ' (',round(variance_explained_pkg_factors_loo[order], 3),')', collapse = ', '))
```
...
get_component_plot <- function(data, component, title) {
  ...
  component_data <- subset(data, Component == component) %>%
    mutate(Chemical = factor(Chemical, levels = desired_order),
           ChemicalLabel = recode(Chemical, !!!chemical_labels))  # Add capitalized label
...
get_component_plot_old <- function(data, component, 
                                   title = paste0('Factor ', component)) {
  col <- color_pal[as.numeric(component)]
  component_data <- subset(data, Component == component)
  component_data <- component_data %>%
    mutate(Chemical = factor(Chemical, levels = desired_order),
           ChemicalLabel = recode(Chemical, !!!chemical_labels))
...
nmfplt_1_svd_5c_less_o3 <- get_component_plot(H_long_5c_less_o3, 
                                           '5', '1) Fugitive and Venting Emssions')
...
nmfplt_4_svd_5c_less_o3 <- get_component_plot(H_long_5c_less_o3, 
                                           '2', '4) General Combustion, Soil Respiration')
nmfplt_5_svd_5c_less_o3 <- get_component_plot(H_long_5c_less_o3, 
                                           '1', '5) Traffic Emissions')
```

```{r}
nmfplt_1_svd_5c_less_o3 <- get_component_plot_old(H_long_5c_less_o3, 
                                           '1')
nmfplt_2_svd_5c_less_o3 <- get_component_plot_old(H_long_5c_less_o3, 
                                           '2')
nmfplt_3_svd_5c_less_o3 <- get_component_plot_old(H_long_5c_less_o3, 
                                           '3')
nmfplt_4_svd_5c_less_o3 <- get_component_plot_old(H_long_5c_less_o3, 
                                           '4')
nmfplt_5_svd_5c_less_o3 <- get_component_plot_old(H_long_5c_less_o3, 
                                           '5')

grid.arrange(nmfplt_1_svd_5c_less_o3,
nmfplt_2_svd_5c_less_o3,
nmfplt_3_svd_5c_less_o3,
nmfplt_4_svd_5c_less_o3,
nmfplt_5_svd_5c_less_o3)
```
...
final_plot
ggsave("factors_patchwork.png", final_plot, width = 16, height = 20)
```
...
fingerprint
ggsave("/Users/meredith/Library/CloudStorage/GoogleDrive-mereditf@usc.edu/Shared drives/HEI Energy/papers/nmf/fingerprint_revised.png", fingerprint)
```
...
get_wind_plot_data <- function(W, factor_names = c('Fugitive and Venting Emissions',
                                                   'General Combustion, Respiration',
                                                   'Flaring I',
                                                   'Flaring II',
                                                   'Traffic'
                                                    )){
...
get_wind_plots <- function(W, y_axis_upper = rep(10, 5),
                               factor_names = c('Fugitive and Venting Emissions',
                                                   'General Combustion, Respiration',
                                                   'Flaring I',
                                                   'Flaring II',
                                                   'Traffic'
  ...
  factor_labels <- setNames(paste(c('1)', '2)','3)', '4)',
                                    ' 5)'), factor_names), paste0('component', 1:5))
...
w
ggsave("/Users/meredith/Library/CloudStorage/GoogleDrive-mereditf@usc.edu/Shared drives/HEI Energy/final report/source-wind-revised.png",w)
```
...
final_plot
ggsave("/Users/meredith/Library/CloudStorage/GoogleDrive-mereditf@usc.edu/Shared drives/HEI Energy/final report/wind_factor_plots_patchwork_final.png", final_plot, width = 8, height = 10)
```