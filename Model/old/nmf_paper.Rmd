...
# knitr::opts_knit$set(root.dir = "/Users/meredith/Library/CloudStorage/GoogleDrive-mereditf@usc.edu/Shared drives/HEI Energy/flaring/STA496-UOGD/STA496-UOGD/")
# load the packages
library(NMF)
library(tidyverse)
library(readxl)
...
```
```{r}
paste('Amount observations where H2S is missing but SO2 is observed:', sum(is.na(hourly_data$h2s) & !is.na(hourly_data$so2)))
paste('Amount observations where SO2 is missing but H2S is observed:', sum(!is.na(hourly_data$h2s) & is.na(hourly_data$so2)))
paste('Amount observations where both H2S and SO2 are missing:', sum(is.na(hourly_data$h2s) & is.na(hourly_data$so2)))
paste('Amount of observations removed due to missing H2S or SO2:', sum(is.na(hourly_data$h2s) | is.na(hourly_data$so2)))
```
...
get_wind_plot_data <- function(W, 
                               factor_names = c('Traffic', 
                                                ...
                                                'Flaring I',
                                                'Flaring II',
                                                'Fugitive and Venting Emissions')){
  data_to_plot <- tibble(
    component1 = W[, 1],
    component2 = W[, 2],
    component3 = W[, 3],
...
get_wind_plots <- function(W,
  ...
  plots <- lapply(factor_order, function(i) {
    ...
    ggplot(
      data_long %>% filter(Factor == factor_name),
      aes(
        x = wd,
        y = Expression,
        fill = as.factor(wd)
...
wind_plot <- get_wind_plots(
  ...
  y_axis_upper = c(0.25, 0.15, 0.15, 0.15, 0.15),
  factor_names = c(
    'Traffic',
    'Combustion/Respiration',
    'Flaring I',
