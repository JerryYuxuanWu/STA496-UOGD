---
title: "NMF Final (Full time period))"
...
plot(lsnmf_random_less_o3_1to3)
```
```{r}
...
```
```{r}
threecomp <- lsnmf_random_less_o3_1to3$fit$`3`
reconstruct<-fitted(threecomp)

# Traditional way of computing WRSS = sum of weight * (X - Xhat)^2
wtss_trad_3 <- sum(weight_matrix * (normalized_matrix_less_o3 - mean(normalized_matrix_less_o3))^2)
wrss_trad_3 <- sum(weight_matrix * (normalized_matrix_less_o3 - reconstruct)^2)
variance_explained_trad_3 <- 1 - (wrss_trad_3 / wtss_trad_3)
variance_explained_trad_3

basis_matrix_5c_less_o3 <- basis(threecomp) #W
coef_matrix_5c_less_o3 <- coef(threecomp) #H
# Incrementally add factors and calculate variance explained

variance_explained_trad_factors <- numeric(3)
variance_explained_pkg_factors <- numeric(3)
...
```{r}
wind_plot <- get_wind_plots(W, y_axis_upper = c(3.5, 1.75, 3.5, 3.75, 3.5))
...
# 14.24 minutes to run the above
...
```{r}
wind_plot <- get_wind_plots(W, y_axis_upper = c(3.5, 1, 2, 2, 4.5))
grid.arrange(grobs = wind_plot, ncol = 3)
```
...
```{r}
wind_plot <- get_wind_plots(W, y_axis_upper = c(0.225, 0.08, 0.18, 0.1, 0.1125))
grid.arrange(grobs = wind_plot, ncol = 3)
```
...
## LSNMF nndsvd with 5 source factors without ozone
