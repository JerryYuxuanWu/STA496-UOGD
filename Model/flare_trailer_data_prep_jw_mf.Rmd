...
for (i in 1:length(radius)){
  if (i == 1){
    wells_monthly <- Wells %>%
      ...
      group_by(substr(prod_date, 1, 7)) %>%
      mutate(inv_dist_well = ifelse(1 / distToLovi < 0.01, 0, 1 / distToLovi)) %>%
      summarize(
        distToLovi_wells = mean(n*distToLovi),
        inv_dist_wells = mean(n*inv_dist_well),
...
flaring_daily <- flaring_loving_50 %>% 
  ...
  # filter(distToLovi_flare <= 100) %>% Not necessary since we already filtered for 50km
  group_by(date) %>% 
...
nmf_data <- readRDS(paste0(wd, 'result_rfiles/normalized_hourly_data_4c_less_o3.rds'))
