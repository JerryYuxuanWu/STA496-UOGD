...
trailer_hourly<-trailer_hourly[which(trailer_hourly$`1_3-butadiene`<=0.1),] # changing this to omit na rows when 1_3-butadiene is missing.
```
...
compute_avg_wind <- function(ws, wd){
  # Compute Wind Vector Azimuth
  # Each wind vector is (u, v) and angle is the clockwise angle from North
  u <- ws * sin(pi/180 * wd)
  v <- ws * cos(pi/180 * wd)
  ...
  mean_direction <- atan2(u_mean, v_mean)*(180/pi)
  
  # Convert to [0, 360]
  mean_direction <- (mean_direction + 360) %% 360 
  return(list(wsp_ms = mean_speed, wdr_deg = mean_direction))
}
...
trailer_daily <- trailer_hourly_clean %>% 
  ...
  summarise(across(!c(wdr_deg, wsp_ms), ~mean(.x, na.rm=T)),
            #wdr_deg = as.numeric(mean(circular(wdr_deg, units = "degrees", template="geographics"), na.rm=T))) %>%
            avg_wind = list(compute_avg_wind(wsp_ms, wdr_deg))) %>%
  unnest_wider(avg_wind)

# Compute nightly average measurement from 0am to 6am (note 10/2/2025 MF change to 5am)
trailer_night_avg <- trailer_hourly_clean %>% 
  filter(hour(ymd_hms(datetime_mountain)) <= 5) %>%
  dplyr::select(-c(time_utc, day_utc, datetime_utc, datetime_mountain)) %>% 
  group_by(day_mountain) %>% 
  summarise(across(!c(wdr_deg, wsp_ms), ~mean(.x, na.rm=T)),
            avg_wind = list(compute_avg_wind(wsp_ms, wdr_deg))) %>%
...
flaring_daily <- flaring_loving_50 %>% 
  mutate(
    inv_dist  = ifelse(1 / distToLovi_flare < 0.01, 0, 1 / distToLovi_flare),
    inv_dist2 = inv_dist^2
  ) %>%
  group_by(date) %>% 
  summarise(
    ...
    inv_dist2_mean         = mean(inv_dist2, na.rm = TRUE),

    # Weighted number of flares
    weighted.count    = sum(inv_dist,  na.rm = TRUE) * 100,
    weighted.count2 = sum(inv_dist2, na.rm = TRUE) * 100,
    ...
    #IDW^2 means
    idw2_imputed_methane_eq = if (sum(inv_dist2, na.rm = TRUE) > 0) {
      sum(imputed_methane_eq * inv_dist2, na.rm = TRUE) / sum(inv_dist2, na.rm = TRUE)
    } else NA_real_,

    idw2_methane_eq = if (sum(inv_dist2, na.rm = TRUE) > 0) {
      sum(methane_eq * inv_dist2, na.rm = TRUE) / sum(inv_dist2, na.rm = TRUE)
    } else NA_real_,

    #IDW^2 totals
    idw2_total_imputed_methane_eq = sum(imputed_methane_eq * inv_dist2, na.rm = TRUE) * 100,
    idw2_total_methane_eq         = sum(methane_eq         * inv_dist2, na.rm = TRUE) * 100,
    ...
    # IDW^2 median
    idw2_median_methane_eq_interp = if (sum(inv_dist2, na.rm = TRUE) > 0) {
      weightedMedian(methane_eq, w = inv_dist2, na.rm = TRUE, interpolate = TRUE)
    } else NA_real_,

    idw2_median_imputed_methane_eq_interp= if (sum(inv_dist2, na.rm = TRUE) > 0) {
      weightedMedian(imputed_methane_eq, w = inv_dist2, na.rm = TRUE, interpolate = TRUE)
    } else NA_real_,
    ...
    methane_eq_median = median(methane_eq, na.rm = TRUE)
  ) %>%
...
flaring_daily <- flaring_daily %>%
  bind_rows(
    tibble(date = as.Date(setdiff(seq(as.Date("2023-05-01"), as.Date("2024-06-01"), by = 'day'), flaring_daily$date)),
           ...
           weighted.count2 = 0,
           methane_eq=0,
           idw2_imputed_methane_eq=0,
           idw2_methane_eq=0,
           idw2_total_imputed_methane_eq=0,
           idw2_total_methane_eq=0,
           idw_total_imputed_methane_eq=0,
           idw_total_methane_eq=0,
           idw2_median_methane_eq_interp=0,
           idw2_median_imputed_methane_eq_interp=0
           )
  ) %>%
  arrange(date)
```
