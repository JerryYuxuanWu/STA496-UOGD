...
trailer_hourly <-readRDS(paste0(wd,"/DataProcessing/Trailer_hourly_merge_20240905.rds"))
...
```{r}
Wells <- readRDS(paste0(wd,"/DataProcessing/WellsProcessed20240601.rds"))
```
...
flaring <- readRDS("/Users/meredith/Library/CloudStorage/GoogleDrive-mereditf@usc.edu/Shared drives/HEI Energy/data/vnf data/final_model_and_dataset/model_dataset_highest_train_r2.rds")
...
flaring_daily <- flaring_loving_50 %>% 
  ...
  group_by(date) %>% 
  summarise(closest.flare = min(distToLovi_flare),
            ...
            imputed_methane_eq_50 = median(imputed_methane_eq, na.rm=T),
...
nmf_data <- readRDS('/Users/meredith/Library/CloudStorage/GoogleDrive-mereditf@usc.edu/Shared drives/HEI Energy/papers/nmf/normalized_hourly_data_4c_less_o3.rds')
...
nmf_factor_hourly <- nmf_data %>%
  dplyr::select(time_utc, Factor1, Factor2, Factor3, Factor4) %>%
...
for (r in radius) {
  ...
  summarise(flare_wd_day = sum(abs(angle - wdr_deg.day) <= 30, na.rm=T),
            ...
            methane_eq = median(methane_eq, na.rm=T),
            imputed_methane_eq = median(imputed_methane_eq, na.rm=T)) %>%
...
write.csv(full_merged, "/Users/meredith/Library/CloudStorage/GoogleDrive-mereditf@usc.edu/Shared drives/HEI Energy/papers/radioactivity/full_data_for_regressions.csv", row.names=FALSE)
