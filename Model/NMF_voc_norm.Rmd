...
```{r}
normalized_matrix <- hourly_nona_bgrm_zerorepl_norm #important: using the normalized VOCs for this file
...
number_row<- dim(transpose_normalized_matrix)[1] #store number of rows (used for checking)
number_column<- dim(transpose_normalized_matrix)[2] #store number of columns
```
...
weight_matrix <- matrix(0, nrow(transpose_normalized_matrix), ncol(transpose_normalized_matrix))
...
LOD_merged <- tibble(chemical = rownames(transpose_normalized_matrix)) %>%
  left_join(LOD_merged)

# creating uncertainty Matrix ???
for (i in 1:number_row) { 
  for (j in 1:number_column) {
    xij <- transpose_normalized_matrix[i, j]
    LOD <- LOD_merged$LOD[[i]]  # Get LOD value for this row 
...
# estimate_rank <- nmfEstimateRank(transpose_normalized_matrix, 4:10, method = "ls-nmf", weight = weight_matrix, 30)
...
output <- nmf(transpose_normalized_matrix, rank = 8, weight = weight_matrix, method = "ls-nmf")
W <- basis(output)
H <- coef(output)
```
...
barplot(W[, 1], names.arg = rownames(transpose_normalized_matrix), main = "Source Contributions to Basis 1", las = 2)
```
```{r}
barplot(W[, 2], names.arg = rownames(transpose_normalized_matrix), main = "Source Contributions to Basis 2", las=2)
```
```{r}
barplot(W[, 3], names.arg = rownames(transpose_normalized_matrix), main = "Source Contributions to Basis 3", las=2)
```
```{r}
barplot(W[, 4], names.arg = rownames(transpose_normalized_matrix), main = "Source Contributions to Basis 4", las=2)
```
```{r}
barplot(W[, 5], names.arg = rownames(transpose_normalized_matrix), main = "Source Contributions to Basis 5", las=2)
```
```{r}
barplot(W[, 6], names.arg = rownames(transpose_normalized_matrix), main = "Source Contributions to Basis 6", las=2)
```
```{r}
barplot(W[, 7], names.arg = rownames(transpose_normalized_matrix), main = "Source Contributions to Basis 7", las=2)
```
```{r}
barplot(W[, 8], names.arg = rownames(transpose_normalized_matrix), main = "Source Contributions to Basis 8", las=2)
```
...
# fitting the optimal rank based on the above plots
# the choice of the optimal rank needs to be discussed
output <- nmf(transpose_normalized_matrix, rank = 4, weight = weight_matrix, method = "ls-nmf")
W <- basis(output)
H <- coef(output)
```
...
```{r}
# Plot source contributions for the first basis vector
nmf4_c1 <- barplot(W[, 1], names.arg = rownames(transpose_normalized_matrix), main = "Source Contributions to Basis 1", las = 2)
```
```{r}
nmf4_c2 <- barplot(W[, 2], names.arg = rownames(transpose_normalized_matrix), main = "Source Contributions to Basis 2", las=2)
```
```{r}
nmf4_c3 <- barplot(W[, 3], names.arg = rownames(transpose_normalized_matrix), main = "Source Contributions to Basis 3", las=2)
```
```{r}
nmf4_c4 <- barplot(W[, 4], names.arg = rownames(transpose_normalized_matrix), main = "Source Contributions to Basis 4", las=2)
```
```{r}
par(mfrow=c(2,2)) 
nmf4_c1
...
# for (n in components) {
#   nmf_result <- nmf(t(transpose_normalized_matrix), rank = n, method = "KL", seed='nndsvd')
#   reconstruction <- basis(nmf_result) %*% coef(nmf_result)
#   error <- norm(t(transpose_normalized_matrix) - reconstruction, type = "F")
#   errors[n-3] <- error
...
```{r, eval=TRUE, echo = F}
# Apply NMF
nmf_result <- nmf(t(transpose_normalized_matrix), rank = 4, method = "KL", seed='nndsvd')
...
grid.arrange(nfmplt_1, nfmplt_2, nfmplt_3, nfmplt_4, ncol=2)
```
