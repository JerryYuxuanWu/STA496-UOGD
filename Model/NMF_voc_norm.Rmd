...
adjusted_hourly <- as.data.frame(sapply(names(hourly_full_nona), function(tracer){ 
  hourly_full_nona[[tracer]] - background_levels[[tracer]]
}))
```

```{r}
# simply checking how many minimum values for each analyte
colSums(adjusted_hourly == 0)
```
```{r}
...
normalized_non_vocs <- as_tibble(sapply(as.list(hourly_non_vocs), normalize_column)) #normalize the NON_VOC
summary(normalized_non_vocs)
```
```{r}
# normalizing the vocs too
normalized_vocs <- as_tibble(sapply(as.list(hourly_vocs), normalize_column))
summary(normalized_vocs)
```

### Combine and Transpose
```{r}
normalized_matrix <- cbind(normalized_non_vocs, normalized_vocs) #important: using the normalized VOCs for this file
