...
trailer_daily <- trailer_ch4_CO_NOx_wind %>% 
  select(-time_utc) %>% 
  group_by(day) %>% 
...
trailer_night_avg <- trailer_ch4_CO_NOx_wind %>% 
  filter(hour(ymd_hms(time_utc)) <= 6 |
           hour(ymd_hms(time_utc)) >= 18) %>%
  select(-time_utc) %>% 
  group_by(day) %>% 
...
for (r in radius) {
  ...
  flare_is_from_wd <- temp %>%
    ...
    left_join(trailer_night_avg, 
              join_by(date == day), suffix = c('.day', '.night')) %>%
    group_by(date) %>%
    summarise(flare_wd_day = any(abs(angle - wdr_deg.day) <= 15),
              flare_wd_night = any(abs(angle - wdr_deg.night) <= 15),
              flare_count = n()) %>%
...
```
```{r}
ch4_nox_5km <- lm(avg_methane_eq_5 ~ ch4.night + co.night + nox.night + wdr_deg.night + wsp_ms.night + flare_wd_night_5 + flare_count_5, data = vnf_trailer_full)
summary(ch4_nox_5km)

ch4_nox_10km <- lm(avg_methane_eq_10 ~ ch4.night + co.night + nox.night + wdr_deg.night + wsp_ms.night + flare_wd_night_10 + flare_count_10, data = vnf_trailer_full)
summary(ch4_nox_10km)

ch4_nox_20km <- lm(avg_methane_eq_20 ~ ch4.night + co.night + nox.night + wdr_deg.night + wsp_ms.night + flare_wd_night_20 + flare_count_20, data = vnf_trailer_full)
summary(ch4_nox_20km)

ch4_nox_50km <- lm(avg_methane_eq_50 ~ ch4.night + co.night + nox.night + wdr_deg.night + wsp_ms.night + flare_wd_night_50 + flare_count_50, data = vnf_trailer_full)
summary(ch4_nox_50km)

ch4_nox_100km <- lm(avg_methane_eq_100 ~ ch4.night + co.night + nox.night + wdr_deg.night + wsp_ms.night + flare_wd_night_100 + flare_count_100, data = vnf_trailer_full)
summary(ch4_nox_100km)
```

```{r}
# Regress concentration of CH4 against wind and flare
ch4_count_20km <- lm(ch4.night ~ wdr_deg.night + wsp_ms.night + flare_wd_night_20 + flare_count_20, data = vnf_trailer_full)
summary(ch4_count_20km)
```
