...
Trailer <- Trailer %>% mutate(datetime = format(as.POSIXct(time_utc), '%Y-%m-%d'))
Trailer <- Trailer %>% mutate(datetime_monthly = format(as.POSIXct(time_utc), '%Y-%m'))
```
```{r}
...
Trailer_daily <- Trailer %>% group_by(datetime) %>% 
  summarize_at(vars(-c(time_utc, hour, datetime_monthly)), list(mean = my_mean))

# add a month variable for that
Trailer_daily <- Trailer_daily %>% mutate(datetime_monthly = format(as.POSIXct(datetime), '%Y-%m'))
```
```{r}
